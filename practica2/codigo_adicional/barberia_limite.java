...
Nota: A veces la frase ¡que pase el siguiente cliente! no aparece.
Esto es porque solo aparece esta frase si se entra en el procedimiento
siguienteCliente(). Hay veces en las que el cliente va a sentarse a esperar
justo en el momento en el que la silla se libera, así que no se sienta y
pasa directamente, con lo que la silla se ocupa y el barbero no pasa al
procedimiento siguienteCliente().
...
class Barberia extends AbstractMonitor{
  ...
  private int limite_clientes = 2;
  private int clientes_esperando = 0;
  public Barberia(){
  ...
  public void cortarPelo(int i){
    enter();
    if(clientes_esperando < limite_clientes){
      ...
      System.out.println("\u001B[32m" + "Cliente "
      + i + " sale de la barbería porque está llena." + "\u001B[0m");
    }
...
class Cliente implements Runnable{
  ...
  public void run(){
    while(true){
      ...
      try{
        Thread.sleep(2500); //el cliente está fuera de la barbería un tiempo.
      }
